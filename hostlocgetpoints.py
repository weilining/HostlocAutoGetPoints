    ...
    def hostloc_get_points(self, usernames, passwords, botToken, chatId):
        ...
        if not usernames or not passwords:
            ...
            try:
                ...
                ip_address = requests.get(url=api_url).text
                self.tg_text += "登录 ip 地址：" + ip_address + "\n"
                print("登录 ip 地址：" + ip_address)
            except Exception as e:
                print("获取登录 ip 地址失败：" + str(e))
                self.text = self.text + "获取登录 ip 地址失败：" + str(e) + "\n"
            self.tg_text += (
                "共检测到{}个帐户，开始获取积分\n".format(len(user_list)) + "*" * 30 + "\n"
            )
            ...
            for i in range(len(user_list)):
                try:
                    ...
                    print("*" * 30)
                continue
            self.tg_text += "程序执行完毕，获取积分过程结束"
            print("程序执行完毕，获取积分过程结束")
        try:
            print("开始推送")
            bot = telegram.Bot(token=botToken)
            bot.send_message(text=self.tg_text, chat_id=chatId)
            print("推送成功")
        except Exception as e:
